...
To preprocess the model result data, and merge different model results into a csv
...
m_thetalist$numParam = 34
m_thetalist[which(m_thetalist$model %in% c("Two-stage Ensemble Mean","Two-stage Ensemble Mean GM" )), "numParam"] = 35
m_thetalist[which(m_thetalist$model %in% c( "EDA", "EDA GM")), "numParam"] = 50
...
par_comb$numParam = 34
par_comb[which(par_comb$model %in% c("Two-stage Ensemble Mean","Two-stage Ensemble Mean GM" )), "numParam"] = 35
par_comb[which(par_comb$model %in% c( "EDA", "EDA GM")), "numParam"] = 50
```
...
```{r}
m_parcomb2$dist_observed = (m_parcomb2$mPSEmap -m_parcomb2$pse)^2 +(m_parcomb2$mJNDmap -m_parcomb2$jnd)^2
mm_parcomb2 <- m_parcomb2 %>% dplyr::group_by(model) %>%
  dplyr::summarise(m_dist_observed = mean(dist_observed)) 
head(mm_parcomb2)
```
