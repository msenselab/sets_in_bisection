...
### compare the predicated results with the GM and AM

```{r}
displayModellist <- c('Bisection Model','Bisection Model GM')

ggplot(par_comb %>% filter(model %in% displayModellist), aes(errpse, errjnd, color = model, shape = model)) + 
  geom_hline(yintercept = 0, linetype='dashed')+ geom_vline(xintercept = 0, linetype='dashed')+ 
  geom_point() +
  xlab('Prediction error in the PSEs (ms)')+
  ylab('Prediction error in the JNDs (ms)')+
...
```{r}
displayModellist <- c('Spacing Model','Spacing Model GM')
...
```{r}
displayModellist <- c('Ensemble Mean','Ensemble Mean GM')
...
```
```{r}
displayModellist <- c('Two-stage Ensemble Mean','Two-stage Ensemble Mean GM')
